...
for svc in order-service risk-service notification-service; do
  ...
  mvn compile com.google.cloud.tools:jib-maven-plugin:3.4.4:build \
    -pl "$svc" \
    -Djib.to.image="$ECR_URI/observability-platform/$svc:latest" \
    -Djib.to.auth.username=AWS \
    -Djib.to.auth.helper=ecr-login \
    -q
  echo "  Pushed $ECR_URI/observability-platform/$svc:latest"
done

# --- 2. Sync config files to EC2 ---
echo ""
echo "--- Syncing config files to EC2 ---"
SSH_OPTS="-i $SCRIPT_DIR/../infra/aws/$KEY_FILE -o StrictHostKeyChecking=no"
...
# Sync prod compose file
scp $SSH_OPTS "$PROJECT_ROOT/docker/docker-compose.prod.yml" ec2-user@"$PUBLIC_IP":"$REMOTE_DIR/docker-compose.prod.yml"
